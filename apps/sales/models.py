...
class Sale(models.Model):
    ...
    date = models.DateTimeField(
        auto_now_add=True,
        help_text=_("Date and time of the sale"),
    )
    total_amount = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        validators=[MinValueValidator(Decimal("0.01"))],
        help_text=_("Total amount of the sale"),
    )
    user = models.ForeignKey(
        ...
        null=True,
        related_name="sales",
        help_text=_("User who processed the sale (cashier)"),
    )
    pharmacy = models.ForeignKey(
        "tenants.Pharmacy",
        on_delete=models.CASCADE,
        related_name="sales",
        help_text=_("Pharmacy where the sale was recorded"),
    )
    notes = models.TextField(
        blank=True,
        null=True,
        help_text=_("Optional notes about the sale"),
    )
...
class SaleItem(models.Model):
    ...
    sale = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        related_name="items",
        help_text=_("Parent sale transaction"),
    )
    pharmacy = models.ForeignKey(
        "tenants.Pharmacy",
        on_delete=models.CASCADE,
        related_name="sale_items",
        help_text=_("Pharmacy where this item was sold"),
    )
    medicine = models.ForeignKey(
        Medicine,
        on_delete=models.PROTECT,
        related_name="sale_items",
        help_text=_("Medicine that was sold"),
    )
    quantity = models.PositiveIntegerField(
        validators=[MinValueValidator(1)],
        help_text=_("Quantity of medicine sold"),
    )
    ...
    subtotal = models.DecimalField(
        ...
        decimal_places=2,
        validators=[MinValueValidator(Decimal("0.01"))],
        help_text=_("Subtotal (quantity x unit price)"),
    )
    created_at = models.DateTimeField(auto_now_add=True)
